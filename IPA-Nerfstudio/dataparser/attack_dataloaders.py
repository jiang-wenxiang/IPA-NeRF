...
class AttackCacheDataloader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.first_ori_time = True
        self.indices = None

        self.cached_collated_batch = None
        self.cached_ori_collated_batch = None
        if self.cache_all_images:
            ...
            self.cached_ori_collated_batch = self._get_collated_batch(ori=True)
    ...
    def getoriitem(self, idx):
        return self.dataset.getoriitem(idx)

    def _get_batch_list(self, ori=False):
        """Returns a list of batches from the dataset attribute."""

        assert isinstance(self.dataset, Sized)
        if ori and self.indices is not None:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            for idx in indices:
                if ori:
                    res = executor.submit(self.dataset.getoriitem, idx)
                else:
    ...
    def _get_collated_batch(self, ori=False):
        """Returns a collated batch."""
        batch_list = self._get_batch_list(ori)
    ...
    def get_ori_iter(self):
        while True:
            if self.cache_all_images:
                collated_batch = self.cached_ori_collated_batch
            elif self.first_ori_time or (
                self.num_times_to_repeat_images != -1 and self.num_repeated >= self.num_times_to_repeat_images
            ):
                # trigger a reset
                collated_batch = self._get_collated_batch(ori=True)
                # possibly save a cached item
                self.cached_ori_collated_batch = collated_batch if self.num_times_to_repeat_images != 0 else None
                self.first_ori_time = False
            else:
                collated_batch = self.cached_ori_collated_batch
            yield collated_batch

    def set_data(self, image_idx, image):
        index = torch.nonzero(self.cached_collated_batch['image_idx'] == image_idx)
        self.cached_collated_batch['image'][index] = image
