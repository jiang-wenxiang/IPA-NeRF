...
from pipeline.attack_pipeline import AttackVanillaPipelineConfig, AttackVanillaPipeline
...
@dataclass
class AttackTrainerConfig(ExperimentConfig):
    """Configuration for training regimen"""

    _target: Type = field(default_factory=lambda: AttackTrainer)
    """target class to instantiate"""

    epsilon: float = 32

    logs_for_all_per_step = 200

    steps_per_save: int = 1000
    """Number of steps between saves."""
    attack_num_iter_per: int = 10
    """Number of steps between saves."""
    ...
    pipeline: AttackVanillaPipelineConfig = AttackVanillaPipelineConfig()
...
class AttackTrainer:
    ...
    pipeline: AttackVanillaPipeline
    optimizers: Optimizers
    callbacks: List[TrainingCallback]

    def __init__(self, config: AttackTrainerConfig, local_rank: int = 0, world_size: int = 1, device: int = -1) -> None:
        self.train_lock = Lock()
        self.config = config
        self.local_rank = local_rank
        self.world_size = world_size
        self.device: TORCH_DEVICE = config.machine.device_type
        if self.device == "cuda" and device >= 0:
    ...
    def attackTrain(self) -> None:
        ...
        attack_num_iter_per = self.config.attack_num_iter_per
        render_iter_per = attack_num_iter_per * 10
        retrain_iter_per = attack_num_iter_per * 20

        self._init_viewer_state()
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            num_iterations = self.config.max_num_iterations
            step = 0
            for step in tqdm.tqdm(range(self._start_step, num_iterations)):
                while self.training_state == "paused":
                    time.sleep(0.01)
                with self.train_lock:
                    with TimeWriter(writer, EventName.ITER_TRAIN_TIME, step=step) as train_t:
                        ...
                        model_before_attack_dict = self.pipeline.model.state_dict()
                        ...
                        # attack train
                        for mini_step in range(attack_num_iter_per):
                            with torch.enable_grad():
                                mini_step_all = step * retrain_iter_per + mini_step
                                ak_loss, ak_loss_dict, ak_metrics_dict, li_loss_dict, li_metrics_dict = \
                                    self.attack_train_iteration(mini_step_all)

                        # close grad update param
                        for n, p in self.pipeline.model.named_parameters():
                            p.requires_grad = False

                        # render p to the train set
                        for mini_step in range(render_iter_per):
                            with torch.no_grad():
                                mini_step_all = step * retrain_iter_per + mini_step
                                self.render_train_set_iteration(mini_step_all, epsilon=self.config.epsilon)
                                torch.cuda.empty_cache()
                                gc.collect()

                        gc.collect()

                        self.pipeline.model.load_state_dict(model_before_attack_dict)
                        # open grad update param
                        for n, p in self.pipeline.model.named_parameters():
                            p.requires_grad = True

                        # time the forward pass
                        for mini_step in range(retrain_iter_per):
                            with torch.enable_grad():
                                mini_step_all = step * retrain_iter_per + mini_step
                                loss, loss_dict, metrics_dict = self.train_iteration(mini_step_all)
                ...
                self.output_image_iteration(step)
    ...
    def render_train_set_iteration(self, step: int, epsilon: float):
        ...
        """
        with torch.no_grad():
            _, loss_dict, metrics_dict = self.pipeline.render_train_set_to_attack(step=step, epsilon=epsilon)
    ...
    @profiler.time_function
    def attack_train_iteration(self, step: int):
        """Run one iteration with a batch of inputs. Returns dictionary of model losses.
        ...
        for _ in range(self.gradient_accumulation_steps):
            with torch.autocast(device_type=cpu_or_cuda_str, enabled=self.mixed_precision):
                _, ak_loss_dict, ak_metrics_dict, _, li_loss_dict, li_metrics_dict = \
                    self.pipeline.get_attack_and_limit_loss_dict(step=step)
                ak_loss = functools.reduce(torch.add, ak_loss_dict.values())
                li_loss = functools.reduce(torch.add, li_loss_dict.values())
                loss = torch.add(ak_loss, li_loss)
                loss /= self.gradient_accumulation_steps
            self.grad_scaler.scale(loss).backward()  # type: ignore
        self.optimizers.optimizer_scaler_step_all(self.grad_scaler)
        ...
        # Merging loss and metrics dict into a single output.
        return loss, ak_loss_dict, ak_metrics_dict, li_loss_dict, li_metrics_dict   # type: ignore
    ...
    @profiler.time_function
    def output_image_iteration(self, step: int):
        writer_log_path = self.base_dir / self.config.logging.relative_log_dir
        self.pipeline.get_image_metrics_and_images_all(step=step, set_name='attack', save_path=writer_log_path)

        if (step + 1) % self.config.logs_for_all_per_step == 0:
            self.pipeline.get_image_metrics_and_images_all(step=step, set_name='train', save_path=writer_log_path)
            self.pipeline.get_image_metrics_and_images_all(step=step, set_name='eval', save_path=writer_log_path)
            self.pipeline.get_image_metrics_and_images_all(step=step, set_name='limit', save_path=writer_log_path)
