...
from configs.attack_method_configs import method_configs
...
def train_loop(device: int, config: AttackTrainerConfig, global_rank: int = 0):
    ...
    trainer = config.setup(device=device)
    trainer.setup()
    trainer.train()


def attack_train_loop(device: int, config: AttackTrainerConfig, global_rank: int = 0):
    """Main training function that sets up and runs the trainer per process
...
def launch(
    ...
    machine_rank: int = 0,
    dist_url: str = "auto",
    config: Optional[AttackTrainerConfig] = None,
    timeout: timedelta = DEFAULT_TIMEOUT,
    device_type: Literal["cpu", "cuda", "mps"] = "cuda",
) -> None:
...
def main(config: AttackTrainerConfig, attack: bool, device: int) -> None:
    """Main function."""
    ...
    if attack:
        ...
        #     config=config,
        # )
        attack_train_loop(device, config)
    else:
        # launch(
        ...
        #     config=config,
        # )
        train_loop(device, config)

def entrypoint():
    """Entrypoint for use with pyproject scripts."""
    # Choose a base configuration and override values.
    methodName = "instant-ngp"
    config = method_configs[methodName]

    data_path = Path("./data/road/road_2")
    attack_flag = True
    device = 0

    # import torch._dynamo
    # torch._dynamo.config.suppress_errors = True
    # torch.TORCH_LOGS = "+dynamo"

    config.steps_per_eval_batch = 500
    config.steps_per_save = 10
    config.logs_for_all_per_step = 150
    config.max_num_iterations = 150

    config.pipeline.datamanager.dataparser.data = data_path
    # config.pipeline.datamanager.train_num_images_to_sample_from = 100
    # config.pipeline.datamanager.train_num_times_to_repeat_images = 100
    # config.pipeline.datamanager.eval_num_images_to_sample_from = 100
    # config.pipeline.datamanager.eval_num_times_to_repeat_images = 100
    config.epsilon = 32.0

    # config.load_dir = Path("./outputs/unnamed/instant-ngp/road_2/nerfstudio_models")

    main(config, attack_flag, device=device)
